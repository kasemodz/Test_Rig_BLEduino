...
# ADS1x15_GND Class
#
# Originally written by K. Townsend, Adafruit (https://github.com/adafruit/Adafruit-Raspberry-Pi-Python-Code/tree/master/Adafruit_ADS1x15_GND)
...
class ADS1x15_GND:
  ...
  def __init__(self, address=0x48, ic=__IC_ADS1015, debug=False):
    ...
    if ((ic < self.__IC_ADS1015) | (ic > self.__IC_ADS1115)):
      if (self.debug):
        print "ADS1x15_GND: Invalid IC specfied: %h" % ic
      return -1
    else:
      self.ic = ic
  ...
  def readADCSingleEnded(self, channel=0, pga=6144, sps=250):
    ...
    if (channel > 3):
      if (self.debug):
        print "ADS1x15_GND: Invalid channel specified: %d" % channel
      return -1
	...
	print "ADS1x15_GND: Invalid pga specified: %d, using 6144mV" % sps     
      config |= self.spsADS1115.setdefault(sps, self.__ADS1115_REG_CONFIG_DR_250SPS)

    # Set PGA/voltage range, defaults to +-6.144V
    if ( (pga not in self.pgaADS1x15_GND) & self.debug):	  
      print "ADS1x15_GND: Invalid pga specified: %d, using 6144mV" % sps     
    config |= self.pgaADS1x15_GND.setdefault(pga, self.__ADS1015_REG_CONFIG_PGA_6_144V)
    self.pga = pga
  ...
  def readADCDifferential(self, chP=0, chN=1, pga=6144, sps=250):
    ...
    if ( (chP == 0) & (chN == 1) ):
      ...
      if (self.debug):
	print "ADS1x15_GND: Invalid channels specified: %d, %d" % (chP, chN)
	return -1
  ...
  def startSingleEndedComparator(self, channel, thresholdHigh, thresholdLow, \
	...
	print "ADS1x15_GND: Invalid sps specified: %d, using 1600sps" % sps       
      config |= self.spsADS1015.setdefault(sps, self.__ADS1015_REG_CONFIG_DR_1600SPS)
    else:
      if ( (sps not in self.spsADS1115) & self.debug):	  
	print "ADS1x15_GND: Invalid sps specified: %d, using 250sps" % sps     
      config |= self.spsADS1115.setdefault(sps, self.__ADS1115_REG_CONFIG_DR_250SPS)

    # Set PGA/voltage range, defaults to +-6.144V
    if ( (pga not in self.pgaADS1x15_GND) & self.debug):	  
      print "ADS1x15_GND: Invalid pga specified: %d, using 6144mV" % pga     
    config |= self.pgaADS1x15_GND.setdefault(pga, self.__ADS1015_REG_CONFIG_PGA_6_144V)
    self.pga = pga
